      ...
      <div className="mx-auto max-w-container px-4">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {textTestimonials.map((item) => (
            <article
              key={item.id}
              className="group relative flex flex-col overflow-hidden rounded-2xl border border-brand-green/10 bg-white p-6 shadow-[0_10px_30px_-18px_rgba(11,60,109,0.35)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_44px_-16px_rgba(11,60,109,0.45)]"
            >
              {/* Decorative quote mark */}
              <div
                aria-hidden="true"
                className="pointer-events-none absolute right-4 top-2 font-serif text-7xl font-bold leading-none text-brand-gold/12 select-none transition-colors duration-300 group-hover:text-brand-gold/20"
              >
                &ldquo;
              </div>
        ...
        <div className="mt-12">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {videoTestimonials.map((item) => (
              <article
                key={item.id}
                className="group overflow-hidden rounded-2xl border border-brand-green/10 bg-white shadow-[0_8px_26px_-16px_rgba(11,60,109,0.3)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_14px_34px_-14px_rgba(11,60,109,0.4)]"
              >
                <a
                ...
                >
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-black/30 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <div className="flex size-10 items-center justify-center rounded-full bg-white/90 shadow-lg">
                      <Play className="size-4 fill-secondary text-secondary" aria-hidden="true" />
                    </div>
                  </div>
                </a>
                <div className="p-4">
        ...
        <div className="mt-10 flex justify-center">
          <Link
            href="/testimonials"
            className="inline-flex items-center gap-2 text-sm font-semibold text-brand-green transition-all hover:gap-3 hover:text-secondary"
          >
